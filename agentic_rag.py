...
def create_rag_graph():
    ...
    # This node retrieve relevant context from the vector database
    @tool(response_format="content_and_artifact")
    def retrieve(query:str):
        """Retrieve information related to a query."""
        retrieved_docs = vectordb.max_marginal_relevance_search(query, k=5, fetch_k=20)
        serialized = "\n\n".join(
            (
    ...
    def query_or_respond(state: MessagesState):
        ...
        tool_decision_prompt = [
            SystemMessage(content=(
                "You are a helpful assistant that answers questions strictly using information from technical documents "
                "about open-source large language models like Mixtral, Phi, DeepSeek, Qwen, Gemma and Llama. "
                "You must use the 'retrieve' tool to answer any question that is specific to these models or their architecture, performance, training, etc. "
                "Only answer directly if the question is completely unrelated to open-source LLMs (like math, general trivia, or weather). "
                "If unsure, always prefer to call the 'retrieve' tool first."
            ))
        ] + state["messages"]
